...
func main() {
	...
	dartRuntimeImport := flags.String("dart_runtime_import", "package:synura/synura.dart", "Dart import path for the runtime library containing the invocation method")
	dartInvokeMethod := flags.String("dart_invoke_method", "invokeBackendAsync", "Dart method name to invoke backend (unary)")
	...
	protogen.Options{
		...
		for _, f := range gen.Files {
			...
			if *lang == "dart" || *lang == "" {
				generateDartFile(gen, f, *dartRuntimeImport, *dartInvokeMethod, serviceList)
			}
			if *lang == "cpp" {
...
func generateGoFile(gen *protogen.Plugin, file *protogen.File, serviceList map[string]bool) {
	...
	for _, service := range file.Services {
		...
		for _, method := range service.Methods {
			if method.Input.GoIdent.GoName == "Empty" || method.Output.GoIdent.GoName == "Empty" {
				usesEmpty = true
				break
			}
		}
	...
	for _, service := range file.Services {
		...
		for _, method := range service.Methods {
			if method.Desc.IsStreamingClient() || method.Desc.IsStreamingServer() {
				g.P(method.GoName, "Internal(context.Context, *", method.Input.GoIdent.GoName, ") (*", method.Output.GoIdent.GoName, ", error)")
			}
		}
	}
	g.P("}")
	...
	for _, service := range file.Services {
		...
		for _, method := range service.Methods {
			...
			typeName := method.Input.GoIdent.GoName
			if typeName == "Empty" {
				typeName = "emptypb.Empty"
			}
			g.P("		req := &", typeName, "{}")
...
func generateDartFile(gen *protogen.Plugin, file *protogen.File, runtimeImport, invokeMethod string, serviceList map[string]bool) {
	...
	g.P("import 'google/protobuf/empty.pb.dart';")
	g.P()
	...
	for _, service := range file.Services {
		...
		for _, method := range service.Methods {
			...
			if !isServerStreaming && !isClientStreaming {
				...
				g.P(fmt.Sprintf("    final resultBytes = await %s('%s', bytes);", invokeMethod, methodName))
				...
				g.P("    final bytes = request.writeToBuffer();")
				g.P(fmt.Sprintf("    return invokeBackendServerStream('%s', bytes)", methodName))
				...
				g.P(fmt.Sprintf("    final resultBytes = await invokeBackendClientStream('%s',", methodName))
				...
				g.P(fmt.Sprintf("    return invokeBackendBidiStream('%s',", methodName))
				g.P("        requests.map((r) => r.writeToBuffer()))")
				g.P("        .map((data) => ", outputType, ".fromBuffer(data));")
				g.P("  }")
			}
