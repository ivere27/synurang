...
if [ ! -f "$EXAMPLE_LITE_TS" ]; then
    echo "Error: example_lite.ts was not generated by default TypeScript mode!"
    exit 1
fi
...
tsc --target ES2020 --module ES2020 --moduleResolution node --strict --noEmit "$EXAMPLE_TS" "$EXAMPLE_LITE_TS"

echo "Generating explicit TypeScript lite code from example.proto..."
protoc -Iexample/api -Iapi -I/usr/include \
    --plugin=protoc-gen-synurang-ffi="$PLUGIN" \
    --synurang-ffi_out="$OUT_DIR" \
