...
*   **C streaming runtime**: Added a shared bounded-queue runtime with a threaded default, non-blocking manual polling for libuv and threadless WebAssembly, typed event callbacks, backpressure, structured errors, retain/release support, and explicit default-runtime shutdown before shared-library unload.
*   **C lite well-known types**: The C codec now supplies `google.protobuf.Timestamp` and `google.protobuf.Duration` alongside `Empty`, so schemas reaching them — including any schema that uses Synurang's own `core.proto` — generate without a protobuf runtime. Other `google.protobuf.*` types are still rejected up front, now naming the supported set. Nested payloads inside a shared well-known message also count against the shared recursion budget instead of restarting it.
*   **C manual runtime drives blocking calls**: On a manual runtime `Synurang_Stream_Send`/`Recv` dispatch pending callbacks on the calling thread until the operation completes, so a stock blocking host can drive a manual or threadless plugin instead of receiving a bare `SYNURANG_PENDING`. `TrySend`/`TryRecv` do not directly pump callbacks, although a synchronous `wakeup` implementation can poll and re-enter before they return.
*   **C enum constant style**: Documented `enum_names=short|qualified`, which selects whether C enum constants keep protobuf's repeated enum-name prefix.
*   **TypeScript async host**: `lang=typescript` also emits `PluginHostAsync` and a `<Service>FfiAsync` client for engines whose work cannot block the caller (a worker, a GPU queue, a network hop). Messages and semantics match the synchronous client; only the call convention differs.
*   **Fixes**: `SYNURANG_INVALID_ARGUMENT` no longer aliases `SYNURANG_ERROR`, so callers can distinguish them and switch over `SynurangStatus`. A generated stream cancelled before its `on_open` runs now passes `NULL` to `on_cancel` and `on_destroy`, preventing shared service state from being mistaken for owned per-stream state. `on_cancel` is for cancellation and async cleanup; `on_destroy` remains the sole final-free point for owned per-stream state. When `on_open` is omitted, `service_user_data` stays shared and borrowed and must not be freed per stream. Stream handle lookup is now bucketed rather than a linear scan of every live stream.
